...
from usuario.form import MiFormularioDeCreacionDeUsuarios,MiFormularioDeEdiconDeDatosDeUsuario
...
def edicion_perfil(request):
    ...
    if request.method =='POST':
        formulario=MiFormularioDeEdiconDeDatosDeUsuario(request.POST,request.FILES,instance=request.user)
        if formulario.is_valid():    
        ...
        formulario=MiFormularioDeEdiconDeDatosDeUsuario(initial={'avatar':info_extra_user.avatar},instance=request.user)
